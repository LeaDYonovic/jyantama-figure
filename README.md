...
- 将本地牌谱和分析结果按雀魂玩家 ID 保存；可从“当前玩家”下拉框切换查看，互不混用断点与统计；
...
4. 打开 Windows 桌面版，点击左侧的“导入本地牌谱数据…”，选择刚下载的文件。文件内含玩家 ID 时会自动归入对应玩家；旧文件没有 ID 时，程序会询问要归入哪个 ID。
...
桌面版 0.9.0 起，导入带玩家 ID 的牌谱 JSON 后，会自动合并保存到：

```text
%LOCALAPPDATA%\MajsoulMortalDesktop\players\<玩家ID>\paipu.json
```

每个 ID 的分析结果分别保存在 `results\majsoul\玩家_<玩家ID>`。左侧“当前玩家”可以直接切换牌谱、断点、PT 和图表；分析某个 ID 时只会检查该 ID 的结果，不会误用其他玩家的数据。重复导入会按 UUID 合并，并保留已有的元数据。
